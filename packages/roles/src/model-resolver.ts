...
import type { MagnitudeModelSpec, MagnitudeConnectionError, MagnitudeStreamError, MagnitudeCallOptions, ModelProfile, RoleId as ClientRoleId } from '@magnitudedev/magnitude-client'
...
export function resolveModel(
  ...
  defaults?: MagnitudeCallOptions,
): BoundModel<{}, MagnitudeConnectionError, MagnitudeStreamError> {
  if (overrides?.[roleId]) {
    const override = overrides[roleId]!
    return override.spec.bind({ auth: override.auth ?? auth, defaults })
  }
