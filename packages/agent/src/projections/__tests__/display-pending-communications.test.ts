  ...
  it('queues inbound pending, mirrors to display, promotes on turn_started, and clears pending', async () => {
    ...
    try {
      ...
      await harness.send({
        ...
        workerRole: 'engineer',
        message: 'do it',
        workerInfo: { agentId: 'agent-1', forkId: 'fork-1', role: 'engineer' },
        timestamp: ts(1),
      } as any)
  ...
  it('does not duplicate promoted inbound messages across subsequent turns and does not leak to root pending', async () => {
    ...
    try {
      ...
      await harness.send({
        ...
        workerRole: 'engineer',
        message: 'do it',
        workerInfo: { agentId: 'agent-2', forkId: 'fork-2', role: 'engineer' },
        timestamp: ts(11),
      } as any)
  ...
  it('routes explicit to="<task-id>" from top-level message', async () => {
    ...
    try {
      ...
      await harness.send({
        ...
        workerRole: 'engineer',
        message: 'do it',
        workerInfo: { agentId: 'agent-explicit', forkId: 'fork-explicit', role: 'engineer' },
        timestamp: ts(20),
      } as any)
