...
export function createHeadlessOutputRenderer() {
  ...
  function handleEvent(event: AppEvent): HeadlessOutput {
    ...
    switch (event.type) {
      ...
      case 'interrupt':
        lines.push(event.allKilled ? dim('■ Interrupted · all agents stopped') : dim('■ Interrupted'))
        break
    }
...
function handleToolEvent(
  ...
  if (event.toolKey === 'spawnWorker') {
    ...
    if (agentId) {
      ...
      const title = outputObject(inner.result)?.title ?? stringValue(record.input.taskId) ?? stringValue(record.input.message)
